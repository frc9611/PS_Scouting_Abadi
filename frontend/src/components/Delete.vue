<script setup lang="ts">
    ...
    const selectedPR = ref<number | null>(null);

    function Menu() {
        window.location.href = '/menu'; // Retorna para o menu
    } 

    function onspan(pr: number) {
        selectedPR.value = pr;

        const element = document.getElementById("deleteMessage") as HTMLElement;
        if (element.className == "no-visible") {
            element.classList.add("deleteMessage"); // adiciona a classe deleteMessage
            element.classList.remove("no-visible"); // remove a classe no-visible
        }
    ...
    function offspan() {
        ...
        const elemento = document.getElementById("deleteMessage") as HTMLElement;
        if (elemento.className == "deleteMessage") {
            elemento.classList.add("no-visible");      // adiciona uma classe
            elemento.classList.remove("deleteMessage"); // remove uma classe
        }
        document.body.style.overflow = 'visible';
    } 

    async function send(pr: any) {
        const data = {
            "NUM": pr,
        }
...
<template>
    <div class="conteudo">
        ...
        >
                <div class="dados">
                    <div class="bigTitle">
                        <h1 class="bigTitleText">Partida {{ item.PR }}:</h1>
                    </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th colspan="2" class="tableTitle">
                                    <button class="DeleteData" @click="onspan(item.PR)">Deletar</button>
                                </th>
                            </tr>
                        </tbody>
                    </table>
            ...
            <div class="buttonDiv">
                <button class="Menu" @click="Menu">Retornar para o menu</button>
            </div>
        </div> 

        <div class="no-visible" id="deleteMessage">
            <div class="text">
                <h6 class="txt1">Pense bem!</h6>
                <h6 class="txt2">Você tem certeza que quer deletar esse item?</h6>
            </div>
            <div class="deleteButtons">
                <button id="yes" class="yes" @click="send(selectedPR)">Sim</button>
                <button id="no" class="no" @click="offspan">Não</button>
            </div>
        </div>
...
.EditData {
    ...
    border-radius: 5px;
    box-shadow: 1px 1px 4px rgba(0, 0, 0, 0.447);
    margin: 10px;
...
/*.yes:hover{
    background-color: rgba(0, 97, 19, 0.627);
}*/

.yes{
    margin-right: 20px;
    color: rgb(0, 0, 0);
    background-color: rgb(193, 255, 197);
}
...
.no:hover{
    color: rgb(255, 255, 255);
    background-color: rgba(97, 0, 0, 0.849);
}
